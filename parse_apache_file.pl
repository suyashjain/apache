...
################################################################################################################################################
	...
	print "Fetching URL...http://$HOSTNAME/server-status\n\n";
    my $prs = Parse::Apache::ServerStatus->new(
        url => 'http://'.$HOSTNAME.'/server-status',
        timeout => 10
    );
    
    my @order    = qw/p r i _ S R W K D C L G I . ta tt rs bs br/; #the output order
    my $interval = 10; # interval in seconds


    print map { sprintf("%7s", $_) } @order;
    
    while ( 1 ) {
        print "\n";
            my $stat = $prs->get or die $prs->errstr;
            print map { sprintf("%7s", $stat->{$_}) } @order;
            sleep($interval);
    }
